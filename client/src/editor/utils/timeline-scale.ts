...
export const ZOOM_CONFIGS: ZoomConfig[] = [
  ...
  { pixelsPerSecond: 10, majorInterval: 30, minorDivisions: 6 },
  { pixelsPerSecond: 20, majorInterval: 10, minorDivisions: 10 },
  { pixelsPerSecond: 40, majorInterval: 5, minorDivisions: 5 },
  { pixelsPerSecond: 80, majorInterval: 1, minorDivisions: 2 },
  { pixelsPerSecond: 160, majorInterval: 1, minorDivisions: 5 },
  { pixelsPerSecond: 320, majorInterval: 0.5, minorDivisions: 5 },
]
...
export function getNextZoom(currentZoom: number): number {
  ...
  return ZOOM_CONFIGS[Math.min(ZOOM_CONFIGS.length - 1, closestIdx + 1)].pixelsPerSecond
}
...
export function getPrevZoom(currentZoom: number): number {
  ...
  return ZOOM_CONFIGS[Math.max(0, closestIdx - 1)].pixelsPerSecond
}
